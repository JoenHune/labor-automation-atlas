...
启动网站后访问 `/labor-automation-atlas/methodology/annotation-qa`，点击“运行 DOM 定位检查”。当前应为 48 / 48 通过；可见结果列出实际选区目标和拒绝原因。新增检查覆盖 NFC 组合字符、韩文组合字母、表情符号、跨节点字素、等价编码变化，以及拒绝不完整字素。矩形字符检查向内收缩 0.1 像素，避开浏览器相邻字形矩形的 1/64 像素重叠。检查结束后删除临时 Markdown 页面，避免将测试入口发布到站点。
