...
function restore(){query.value=new URLSearchParams(location.search).get('filter.task')??''}
async function load(){if(index.value||loading.value||!query.value.trim())return;loading.value=true;error.value='';try{const response=await fetch(withBase('/exports/task-search-'+props.country+'.json'));if(!response.ok)throw new Error();const data=await response.json() as SearchIndex;if(data.country!==props.country||!Array.isArray(data.tasks)||!Array.isArray(data.industries)||data.tasks.some((t:TaskCard)=>t.country!==props.country))throw new Error();index.value=data}catch{error.value='任务索引暂时无法加载，请重试'}finally{loading.value=false}}
watch(query,()=>{limit.value=30;if(typeof window==='undefined')return;const url=new URL(location.href);if(query.value)url.searchParams.set('filter.task',query.value);else url.searchParams.delete('filter.task');history.replaceState(null,'',url);window.dispatchEvent(new CustomEvent('atlas:view-change'));load()})
