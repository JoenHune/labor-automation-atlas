...
<template>
 <article class="task-view" :data-country="task.country">
  ...
  <section :data-content-id="task.id+'-economics'" tabindex="0"><h2>回报：参数不足</h2><p class="work-note">未生成回收期或净现值。原始计价币种为 {{task.country==='cn'?'人民币（CNY）':'美元（USD）'}}，先补齐可避免的付薪投入、完整部署费用、维护和异常停机，再检验需求与产能。</p>
