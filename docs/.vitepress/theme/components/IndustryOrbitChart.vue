...
<template>
 <section ref="host" class="orbit" :id="country+'-ranking-chart'" :data-content-id="country+'-orbit-navigation-'+root.year+'-'+(basis||'annual')" @keydown.esc="back" @pointermove="hoverBlocked=false">
  ...
  <footer class="orbit-footer"><p><span class="gesture-icon">↗</span> 镜头走近，扇区角度保持不变</p><div><button @click="share">分享此视图</button><button @click="exportSvg">导出图表</button><button @click="exportData">数据与来源 ↓</button></div></footer>
