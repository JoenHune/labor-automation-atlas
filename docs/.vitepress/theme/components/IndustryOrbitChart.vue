...
import {employmentView,employmentExport} from '../../../../src/research/employment'
...
const emit=defineEmits<{'update:year':[year:number];'retry-employment':[]}>()
const {settings,unit}=useCurrency()
const basis=ref(''),focusId=ref(''),detailId=ref(''),hoverId=ref(''),notice=ref('')
type OrbitAnalysisTab=AnalysisSection|'employment'
const analysisTab=ref<OrbitAnalysisTab>('structure')
...
const employmentPeers=computed(()=>{const parent=active.value.children.length?active.value:index.value.get(active.value.parentId??'')??focus.value;const l=orbitLayer(parent);return [...l.sectors,...l.missing,...l.nonpositive]})
const availableAnalysis=computed<OrbitAnalysisTab[]>(()=>[...analysisSections(props.research.industryAnalysis,props.country,active.value.id),...(props.research.employment||props.employmentState?['employment' as const]:[])])
const selectedAnalysis=computed(()=>availableAnalysis.value.includes(analysisTab.value)?analysisTab.value:'structure')
const analysisLabels:Record<OrbitAnalysisTab,string>={structure:'组成与分配',regions:'地区分布',companies:'企业经营',employment:'就业与人均'}
...
function reframe(){if(!chart||cameraMode.value!=='focus')return;const target=visibleSectors.value.some(s=>s.node.id===active.value.id)?active.value.id:[...path.value].reverse().find(n=>visibleSectors.value.some(s=>s.node.id===n.id))?.id??root.value.id;const camera=orbitCamera(bands.value,target,chart.getWidth(),chart.getHeight());viewZoom.value=camera.zoom;panX.value=camera.x;panY.value=camera.y;void nextTick(anchors)}
...
function restore(){
 ...
 analysisTab.value=['regions','companies','employment'].includes(query.get('filter.analysis')??'')?query.get('filter.analysis') as OrbitAnalysisTab:'structure'
...
function changeAnalysis(value:OrbitAnalysisTab){analysisTab.value=value;sync()}
function historical(){
  ...
  <div class="orbit-layout">
   ...
   <aside class="orbit-inspector" :data-content-id="focus.id+'-orbit-inspector-'+root.year">
    ...
    <template v-if="detail">
     ...
     <EmploymentSummary v-if="research.employment" :dataset="research.employment" :node="detail" :basis="basis"/>
     <button v-if="research.employment" class="employment-entry" @click="changeAnalysis('employment')">比较就业与人均 · 查看估算依据 →</button>
    ...
    <template v-else>
     ...
     <EmploymentSummary v-if="research.employment" :dataset="research.employment" :node="focus" :basis="basis"/>
     <button v-if="research.employment" class="employment-entry" @click="changeAnalysis('employment')">比较就业与人均 · 查看估算依据 →</button>
    ...
    <template v-if="selectedAnalysis==='employment'">
     <EmploymentPanel v-if="research.employment" :research="research" :node="active" :peers="employmentPeers" :basis="basis" @select="choose" @hover="hoverBlocked=false;emphasize($event)" @clear="clearHover"/>
     <p v-else role="status" class="detail-gap">{{employmentState==='failed'?employmentError:'正在读取就业人数与估算依据…'}} <button v-if="employmentState==='failed'" @click="emit('retry-employment')">重新读取</button></p>
    </template>
    <IndustryAnalysisPanel v-else-if="analysisReady&&research.industryAnalysis" :research="research" :country="country" :basis="basis" :node-id="active.id" :node-name="active.name" :section="selectedAnalysis"/>
...
<style scoped>
.inspector-analysis-tabs{display:flex;flex-wrap:wrap;gap:4px 15px;border-bottom:1px solid #dce4eb;margin-bottom:15px}.inspector-analysis-tabs button{font-size:12px;color:#708398;padding:7px 0 10px;border-bottom:2px solid transparent;white-space:nowrap}.inspector-analysis-tabs button[aria-pressed=true]{color:#244f7b;border-color:#426e9e}.employment-entry{font-size:11px;color:#537da2;margin:0 0 14px;padding:2px 0}.orbit-hover-preview:has(.employment-summary){max-width:310px;width:min(310px,calc(100% - 28px))}
