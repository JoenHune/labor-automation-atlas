...
import {macroStructure,macroScale} from '../../../../src/research/macro-structure'
...
const chartYear=computed(()=>structure.value?.year??props.year)
const roots=computed(()=>structure.value?macroScale(props.research,props.country,basis.value):industryScale(props.research,props.country,props.year))
const title=computed(()=>structure.value?.title??props.year+' 年行业规模')
...
const balanceNotes=computed(()=>rows.value.filter(r=>r.expanded&&r.node.value!==null&&r.node.children.every(c=>c.value!==null)).flatMap(({node})=>{
 const delta=node.children.reduce((n,c)=>n+(scaleAmount(c,settings.value)??0),0)-(scaleAmount(node,settings.value)??0)
 return Math.abs(delta)>1e-7?[`${node.name}：子项合计与父项原值相差 ${formatAmount(Math.abs(delta),4)} ${unit.value}，保留原表数值，不调整子项。`]:[]
}))
...
function setBasis(id:string,target=''){
 basis.value=id;expanded.value=new Set();selectedId.value=''
 const node=index.value.get(target)
 if(node){const state=scaleState(index.value,new URLSearchParams('filter.scaleDetail='+target));expanded.value=state.expanded;selectedId.value=state.selected}
 sync()
 void nextTick(()=>{host.value?.scrollIntoView({behavior:'smooth',block:'start'});draw()})
}
function historicalDetails(){
 const node=selected.value
 let target='cn-io-industry'
 if(node?.id==='cn-manufacturing')target='cn-io-manufacturing'
 else if(node?.id==='cn-mining')target='cn-io-mining'
 else if(node?.id==='cn-utilities')target='cn-io-utilities'
 else if(node?.id.startsWith('cn-gbt2017-')){
  const prefix=node.id.slice(-2),view=macroStructure(props.research,props.country,'cn-io2023')
  target=view?.nodes.find(n=>n.memberCodes.every(c=>c.startsWith(prefix)))?.id??target
 }
 setBasis('cn-io2023',target)
}
function sync(){
...
function restore(){
 restoring=true
 const requested=new URLSearchParams(location.search).get('filter.scaleBasis')??''
...
function expandSelected(){
 const next=new Set(expanded.value);let node=selected.value
 while(node){if(node.children.length)next.add(node.id);node=node.parentId?index.value.get(node.parentId):undefined}
 expanded.value=next;close()
}
...
function syncAnchors(){
 ...
 const points=rows.value.filter(r=>!r.expanded&&r.node.baseValue!==null).map(r=>({key:r.node.id,period:String(chartYear.value),value:r.node.value}))
 ...
 host.value.dataset.annotationPointRects=JSON.stringify(rows.value.flatMap((r,i)=>{
  ...
  return [{key:r.node.id,period:String(chartYear.value),value:r.node.value,rect:{x:box.x-outer.x+Math.min(zero,pos[0]),y:box.y-outer.y+pos[1]-10,width:Math.max(1,Math.abs(pos[0]-zero)),height:20}}]
 }))
}
function draw(){
 if(!chart)return
 ...
 chart.setOption({animation:false,textStyle:{fontFamily:'system-ui,sans-serif'},
  ...
  tooltip:{trigger:'item',confine:true,renderMode:'richText',formatter:(p:any)=>{
   ...
   return n.name+'\n'+chartYear.value+' 年 · '+money(n)+' '+unit.value+(share===null?'':'\n占'+index.value.get(n.parentId!)?.name+' '+pct(share))
  }},
  ...
  yAxis:{type:'category',inverse:true,data:list.map(r=>r.node.id),triggerEvent:true,axisLine:{show:false},axisTick:{show:false},axisLabel:{interval:0,fontSize:compact.value?11:12,width:left-16,overflow:'break',lineHeight:16,formatter:(id:string)=>{
   ...
   return (r.depth?'　'.repeat(Math.min(r.depth,4))+'↳ ':r.node.children.length?(r.expanded?'▾ ':'▸ '):'')+r.node.name
  },color:'#263548'}},
...
function exportData(){
 ...
 const record={basis:structure.value?.id??'annual-industry',structure:structure.value?{title:structure.value.title,description:structure.value.description,year:structure.value.year,version:props.research.macroStructures?.version,checkedAt:props.research.macroStructures?.checkedAt}:null,version:props.research.version,checkedAt:props.research.checkedAt,country:props.country,year:chartYear.value,settings:settings.value,expanded:[...expanded.value],selected:selectedId.value,visibleRows:visible,nodes,sources:sources.value.filter(s=>refs.has(s.id)),note:'分组总量不与子行业重复相加；未分列规模为总量减已列子项，未分配给缺值行业。'}
 const a=document.createElement('a'),url=URL.createObjectURL(new Blob([JSON.stringify(record,null,2)],{type:'application/json'}));a.href=url;a.download=props.country+'-industry-scale-'+chartYear.value+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)
}
function exportSvg(){if(chart)downloadChartSvg(chart,countryName.value+' · '+title.value,(structure.value?'2023 年产品部门 · 历史资料 · ':'')+unit.value+' · 同一金额刻度 · 1美元='+settings.value.usdCny+'人民币 · 分组总量不重复计入',props.country+'-industry-scale-'+chartYear.value)}
...
<template>
 <section ref="host" :id="country+'-ranking-chart'" :data-content-id="country+'-industry-scale-v2-'+chartYear+(structure?'-'+structure.id:'')" class="scale-section" tabindex="0" @keydown="keydown">
  <div class="scale-heading"><div><p class="scale-eyebrow">从总量看组成</p><h2>{{title}}</h2></div><span>{{structure?'当年生产者价格增加值':'现价增加值'}} · {{unit}}</span></div>
  <div v-if="country==='cn'&&research.macroStructures?.structures.length" class="scale-basis" role="group" aria-label="行业图谱统计口径"><button :aria-pressed="!structure" @click="setBasis('')">{{year}} 年行业增加值</button><button :aria-pressed="!!structure" @click="setBasis('cn-io2023')">2023 年产品部门结构</button></div>
  <p v-if="structure" class="scale-gap structure-notice">{{structure.description}}</p>
  <p v-if="country==='cn'" class="scale-footnote"><a :href="withBase('/macro-data')">宏观数字、统计口径与剩余缺口</a></p>
  ...
  <div ref="chartEl" :style="{height:height+'px'}" class="scale-chart" tabindex="0" role="img" :aria-label="countryName+' '+chartYear+' 年'+(structure?'产品部门结构':'行业规模')+'，可展开的同刻度柱状图'"></div>
  <p v-if="expanded.size" class="scale-footnote">浅蓝色为展开的细分行业，所有柱沿用同一金额刻度。分组标题显示总量，不重复计入子行业。</p>
  <p v-for="note in balanceNotes" :key="note" class="scale-footnote">{{note}}</p>
  <div v-if="gapsVisible" class="scale-gap"><span>部分细分尚无 {{chartYear}} 年金额。灰柱为总量减已列子项的未分列规模，不代表某个具名行业。</span><button v-if="!structure&&completeYears.length" @click="emit('year',completeYears[0])">查看 {{completeYears[0]}} 年已列门类金额</button></div>
  ...
  <aside v-if="selected" ref="drawer" role="dialog" :aria-labelledby="country+'-scale-detail-title'" class="scale-drawer" :data-content-id="selected.id+'-scale-detail-'+chartYear" tabindex="-1">
   <div class="drawer-top"><span>{{countryName}} · {{chartYear}} 年{{structure?' · 产品部门':''}}</span><button ref="closeButton" aria-label="关闭行业详情" @click="close">×</button></div>
   ...
   <div class="drawer-metrics"><div><span>{{structure?'产品部门增加值':'现价增加值'}}</span><strong>{{money(selected)}}</strong><small>{{unit}} · {{chartYear}} 年</small></div><div v-if="parent"><span>占{{parent.name}}</span><strong>{{pct(selectedShare)}}</strong><small>同年增加值 ÷ 上级总量</small></div></div>
   ...
   <section class="drawer-children"><div class="children-heading"><h3>下一级组成</h3><span v-if="selected.children.length">{{selected.children.length}} 项</span></div>
    ...
    <template v-else><p v-if="selected.children.every(n=>n.value===null)" class="scale-footnote">已列出分类名称；{{chartYear}} 年金额待补，暂不绘制比例。</p>
     ...
     <ul><li v-for="node in selected.children" :key="node.id" :data-content-id="node.id+'-scale-child-'+chartYear"><button @click="details(node.id)"><span>{{node.name}}</span><span class="child-number">{{money(node)}}<small>{{node.value===null?'金额待补':pct(scaleShare(node,selected))}}</small></span><span aria-hidden="true">›</span></button></li></ul>
...
<style scoped>
.scale-basis{display:flex;flex-wrap:wrap;gap:6px;margin:18px 0 10px}.scale-basis button{border:1px solid #dce3ee;border-radius:6px;padding:8px 12px;font-size:12px;color:#53647b}.scale-basis button[aria-pressed="true"]{background:#eaf1ff;border-color:#8db3ed;color:#245cb5}.structure-notice{border-left:3px solid #2461d5}
